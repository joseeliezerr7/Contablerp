...
<div>
    ...
    <div class="mb-4 flex items-center justify-between">
        <h2 class="text-lg font-semibold">
            {{ $transferId ? 'Editar traslado' : 'Nuevo traslado entre bodegas' }}
        </h2>
    ...
    <div class="mb-4 rounded-xl border border-slate-200 bg-white p-4">
        <div class="grid gap-4 md:grid-cols-4">
            ...
            <x-field label="Desde" for="from_warehouse_id" error="from_warehouse_id">
                <select id="from_warehouse_id" wire:model.live="from_warehouse_id" class="{{ $input }}">
            ...
            <x-field label="Hacia" for="to_warehouse_id" error="to_warehouse_id">
                <select id="to_warehouse_id" wire:model="to_warehouse_id" class="{{ $input }}">
                    @foreach ($warehouses as $warehouse)
                        <option value="{{ $warehouse->id }}">{{ $warehouse->label() }}</option>
                    @endforeach
                </select>
        ...
        <div class="mt-4">
            <x-field label="Notas" for="notes" error="notes">
                <input id="notes" type="text" wire:model="notes" class="{{ $input }} w-full">
            </x-field>
        </div>
    </div>
        ...
        <table class="table-stacked w-full text-sm">
            <thead role="rowgroup" class="border-b border-slate-200 bg-slate-50 text-left text-xs tracking-wider text-slate-500 uppercase">
                <tr role="row">
                    ...
                    <th role="columnheader" class="w-40 px-3 py-2 text-right font-semibold">Disponible en origen</th>
                    <th role="columnheader" class="w-32 px-3 py-2 text-right font-semibold">Cantidad</th>
            ...
            <tbody role="rowgroup" class="divide-y divide-slate-100">
                @foreach ($lines as $index => $line)
                    <tr role="row">
                        ...
                        <td role="cell" data-label="Disponible" class="px-3 py-1.5 text-right font-mono text-slate-500">
                            {{ $trim($line['on_hand'] ?? null) ?? '—' }}
                        </td>
                        <td role="cell" data-label="Cantidad" class="px-3 py-1.5">
    ...
    <p class="mt-3 text-xs text-slate-500">
        El costo viaja con la mercadería: la bodega que recibe la registra al mismo costo que
        tenía en la de origen. Un traslado no genera partida contable.
    </p>
