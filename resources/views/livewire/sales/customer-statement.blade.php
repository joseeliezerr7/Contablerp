...
<div>
    ...
    <div class="mb-4">
        ...
        <p class="text-sm text-slate-500">Cargos, abonos y saldo acumulado de un cliente.</p>
    </div>

    <div class="mb-4 flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4">
        <label class="text-sm">
            <span class="mb-1 block font-medium text-slate-700">Cliente</span>
            <select wire:model.live="customerId" class="{{ $input }} w-72">
            ...
            <table class="table-stacked w-full text-sm">
                <thead role="rowgroup" class="border-b border-slate-200 bg-slate-50 text-xs tracking-wider text-slate-500 uppercase">
                    <tr role="row">
                        ...
                        <th role="columnheader" class="px-4 py-2 text-right font-semibold">Abono</th>
                ...
                <tfoot role="rowgroup" class="border-t-2 border-slate-300 bg-slate-50 font-semibold">
                    <tr role="row">
                        <td role="cell" colspan="5" class="px-4 py-2 text-right">Saldo al {{ \Carbon\Carbon::parse($to)->format('d/m/Y') }}</td>
