...
<div wire:keydown.window.alt.n.prevent="addLine">
    ...
    <div class="mb-4 flex items-center justify-between">
        <div>
            <h2 class="text-lg font-semibold">{{ $purchaseId ? 'Editar compra' : 'Nueva compra' }}</h2>
            <p class="text-sm text-slate-500">
                Los precios los dicta la factura del proveedor; se teclean tal como vienen.
            </p>
        </div>
        <a href="{{ route('purchases.index') }}" wire:navigate
           class="text-sm text-slate-600 underline hover:text-slate-900">Volver a compras</a>
    </div>
    ...
    <form wire:submit="saveAndReceive" class="space-y-4">
        <div class="grid gap-4 rounded-xl border border-slate-200 bg-white p-4 sm:grid-cols-4">
            ...
            <x-field label="Factura del proveedor" for="supplier_invoice_number" error="supplier_invoice_number">
                <input id="supplier_invoice_number" type="text" wire:model="supplier_invoice_number"
                       class="{{ $input }} font-mono" placeholder="000-001-01-00000001">
            </x-field>
            ...
            <x-field label="Bodega que recibe" for="warehouse_id" error="warehouse_id">
                <select id="warehouse_id" wire:model="warehouse_id" class="{{ $input }}">
                    <option value="">Sin bodega (solo gastos)</option>
        ...
        <div class="table-stacked-wrap overflow-x-auto rounded-xl border border-slate-200 bg-white">
            <table class="table-stacked w-full text-sm">
                <thead role="rowgroup" class="border-b border-slate-200 bg-slate-50 text-left text-xs tracking-wider text-slate-500 uppercase">
                    <tr role="row">
                        ...
                        <th role="columnheader" class="w-28 px-2 py-2 font-semibold">Código</th>
                        <th role="columnheader" class="px-2 py-2 font-semibold">Descripción</th>
                        <th role="columnheader" class="w-40 px-2 py-2 font-semibold">Cuenta de gasto</th>
                        <th role="columnheader" class="w-20 px-2 py-2 text-right font-semibold">Cant.</th>
                        <th role="columnheader" class="w-28 px-2 py-2 text-right font-semibold">Costo</th>
                        <th role="columnheader" class="w-20 px-2 py-2 text-right font-semibold">Desc.%</th>
                        <th role="columnheader" class="w-28 px-2 py-2 font-semibold">Impuesto</th>
                ...
                <tbody role="rowgroup" class="divide-y divide-slate-100">
                    @foreach ($lines as $index => $line)
                        ...
                        @endphp
                        <tr role="row" wire:key="purchase-line-{{ $index }}">
                            ...
                            <td role="cell" data-label="Código" class="px-2 py-1">
                                <input type="text" list="productos-compra"
                            ...
                            <td role="cell" data-label="Cuenta" class="px-2 py-1">
                                <select wire:model="lines.{{ $index }}.expense_account_id" class="{{ $cell }}">
                                    <option value="">Automática</option>
                                    @foreach ($expenseAccounts as $expenseAccount)
                                        <option value="{{ $expenseAccount->id }}">{{ $expenseAccount->code }} — {{ $expenseAccount->name }}</option>
                                    @endforeach
                                </select>
                            </td>
                            ...
                            <td role="cell" data-label="Costo" class="px-2 py-1">
                                <input type="text" inputmode="decimal"
            ...
            <div class="flex items-start justify-between border-t border-slate-200 px-3 py-2">
                ...
                <dl class="w-64 space-y-1 text-sm">
                    ...
                    @endif
                    <div class="flex justify-between">
                        <dt class="text-slate-500">Impuesto acreditable</dt>
